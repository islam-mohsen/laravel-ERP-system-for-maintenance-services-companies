            ...
            <div class="card-body">
                <div class="row">
                    <div class="offset-sm-2 col-sm-8">
                        ...
                        <input id="roomname" class="form-control" placeholder="Add Room">
                    </div>
                </div>
                <br>
                <br>
...
<div class="card-body">
    ...
    @endif
    @if(session()->has('addedstore'))
        <div class="alert alert-success">{{session()->get('addedstore')}}</div>
    @endif
    @if(session()->has('deletestore'))
        <div class="alert alert-success">{{session()->get('deletestore')}}</div>
    @endif
    <form action="{{route('store.add.post')}}" method="post" enctype="multipart/form-data" id="prod">
        @csrf
        <div id="formdata" class="form-group">
            <label for="invoicenumber">Store Name :</label>
            <input type="text" class="form-control" id="storename" name="storename" aria-describedby="emailHelp"
            ...
            <table class="table table-dark">
                ...
                @foreach($store as $store)
                    ...
                    <tr>
                        ...
                        <td>{{$store->name}}</td>
                        <td>
                            <a href='{{route('room.add.get',[$store->id])}}'>
                                <button type='button' class='btn btn-primary'>Edit</button>
                            </a>
                        </td>
                        <td>
                            <a href='{{route('store.delete.get',[$store->id])}}'>
