...
@section('content')
    ...
    <div class="card-body">
        ...
        @endif
        @if(session()->has('updateMachine'))
            <div class="alert alert-success">{{session()->get('updateMachine')}}</div>
        @endif

          <form method="POST" action="{{route('updateMachine',$update->id)}}" enctype="multipart/form-data">
              @csrf
             @method('PUT')
             <div class="form-row">
